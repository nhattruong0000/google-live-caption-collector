...
const extensionStatusJSON_bug = {
  "status": 400,
  "message": "<strong>Google live caption collector encountered a new error</strong>."
}
const reportErrorMessage = "There is a bug in Google live caption collector."
...
async function checkExtensionStatus() {
  ...
  chrome.storage.local.set({
    extensionStatusJSON: { status: 200, message: "<strong>Google live caption collector is running</strong> <br /> Do not turn off captions" },
  });
}
...
function meetingRoutines(uiType) {
  ...
  checkElement(meetingEndIconData.selector, meetingEndIconData.text).then(() => {
    ...
    try {
      ...
      chrome.storage.sync.get(["operationMode"], function (result) {
        if (result.operationMode == "manual")
          showNotification({ status: 400, message: "<strong>Google live caption collector is not running</strong> <br /> Turn on captions using the CC icon, if needed" })
        else
          showNotification(extensionStatusJSON)
      })
