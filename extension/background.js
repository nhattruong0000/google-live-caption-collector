...
function downloadTranscript() {
    chrome.storage.local.get(["userName", "transcript", "chatMessages", "meetingTitle", "meetingStartTimeStamp"], function (result) {
        if (result.userName && result.transcript && result.chatMessages) {
            ...
            const fileName = result.meetingTitle && result.meetingStartTimeStamp ? `transcript/Transcript-${result.meetingTitle} at ${result.meetingStartTimeStamp}.txt` : `transcript/Transcript.txt`
            ...
            reader.onload = function (event) {
                ...
                chrome.downloads.download({
                    ...
                    console.log("Transcript downloaded to transcript directory")
                }).catch((error) => {
                    console.log(error)
                    chrome.downloads.download({
                        url: dataUrl,
                    ...
                    console.log("Invalid file name. Transcript downloaded to transcript directory with simple file name.")
                })
            }
